...
export class ApiService {
  ...
  getProduct(id: number) {
    return this.httpClient.get(this.apiUrl + '/products/' + id).pipe(
      map(res => res['product'])
    )
  }
  
  createProduct(product: ProductModel) {
    return this.httpClient.post(this.apiUrl + '/products/', product).pipe(
      map(res => res['product'])
    )
  }
//not completed, waiting on api
  updateAllProducts(test: string) {
    return this.httpClient.put(this.apiUrl + '/products/', test).pipe(
      map(res => res['product'])
    )
  }
//api failing
